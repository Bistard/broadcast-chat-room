...
int client() {
    ...
    if (client_sock == -1) 
        Logger::log_error("client_sock erorr", true);
    ...
    if (connect(client_sock, 
        ...
        Logger::log_error("server connection error", true);
    }
    ...
    while (true) {
        ...
        for (int i = 0; i < success_event_count; i++) {
            ...
            if (sock_fd == client_sock) {
                ...
                if (msg_size == -1) 
                    Logger::log_error("client cannot read message", true);
                
                // reading name
                char name_buffer[50];
                int name_size = read(client_sock, name_buffer, 50);
                if (name_size == -1) 
                    Logger::log_error("client cannot read name", true);
...
static void read_input(Chatroom *chat) {
    ...
    Logger::log_error("expected exit", true);
}
